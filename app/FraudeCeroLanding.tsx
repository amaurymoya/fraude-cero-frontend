...
const FIGMA_URL =
  'https://www.figma.com/make/BFdP8dbOzbDeNRYQlqBTm9/Mockup-App-Fraude-Cero?node-id=0-1&p=f&fullscreen=1'
const API_DOCS_URL =
...
export default function FraudeCeroLanding() {
  ...
  // <-- ESTAS DECLARACIONES DE ESTADO DEBEN IR ANTES DE USAR setError -->
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      <header className="sticky top-0 z-50 backdrop-blur bg-white/70 border-b border-neutral-200">
        <nav className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a className="px-3 py-2 rounded-xl text-sm font-medium bg-neutral-900 text-white" href={FIGMA_URL} target="_blank">
              Ver Figma
            </a>
            <a className="px-3 py-2 rounded-xl text-sm font-medium border border-neutral-300" href={API_DOCS_URL} target="_blank">
              API Docs
            </a>
      ...
      <section className="mx-auto max-w-6xl px-4 py-8">
        ...
        <p className="mt-2 text-neutral-700">
          Analiza mensajes, entrega un score de riesgo y sugiere acciones simples.
        </p>
      </section>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {examples.map((ex, i) => (
                <button
                  key={i}
                  onClick={() => setMsg(ex)}
                  className="px-3 py-2 rounded-xl text-xs border border-neutral-300"
                >
                  Ejemplo {i + 1}
                </button>
              ))}
